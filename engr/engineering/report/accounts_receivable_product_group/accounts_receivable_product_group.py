...
class ReceivablePayableReport:
	...
	def set_invoice_details(self, row):
		...
		if row.voucher_type == "Sales Invoice":
			...
			if row.payment_due_date:
				row.over_due_days = (
					getdate(nowdate()) - getdate(row.payment_due_date)
				).days
			else:
				row.over_due_days = 0
	...
	def get_invoice_details(self):
		...
		if self.account_type == "Receivable":
			si_list = frappe.db.sql(
				"""
				select name, due_date, po_no,pdc_date,pdc_amount,pdc_number
			...
			sales_contribution = frappe.db.sql("""
				SELECT
					parent,
					GROUP_CONCAT(DISTINCT product_group SEPARATOR ', ') as product_group
				FROM `tabSales Team`
				GROUP BY parent
			""", as_dict=1)

			for d in sales_contribution:
				if d.parent in self.invoice_details:
					self.invoice_details[d.parent]["product_group"] = d.product_group
	...
	def add_customer_filters(
		...
		if self.filters.get("product_group"):
			product_groups = self.filters.get("product_group")

			if isinstance(product_groups, str):
				product_groups = [product_groups]

			self.qb_selection_filter.append(
				self.ple.voucher_no.isin(
					qb.from_(qb.DocType("Sales Invoice"))
					.select(qb.DocType("Sales Invoice").name)
					.where(
						qb.DocType("Sales Invoice").name.isin(
							qb.from_(qb.DocType("Sales Team"))
							.select(qb.DocType("Sales Team").parent)
							.where(
								qb.DocType("Sales Team").product_group.isin(product_groups)
							)
						)
					)
				)
	...
	def get_columns(self):
		...
		self.add_column(
		label=_("Over Due Days"),
		fieldname="over_due_days",
		fieldtype="Int",
		width=120,
		)
		...
		self.add_column(label=_("PDC Date"), fieldname="pdc_date", fieldtype="Date")
		self.add_column(label=_("PDC Amount"), fieldname="pdc_amount")
		self.add_column(label=_("PDC Number"), fieldname="pdc_number", fieldtype="Data")
