...
def execute(filters=None):
	...
	if filters.get('show_other_data'):
		columns, earning_types, ded_types = get_columns(filters, salary_slips)
	else:
		columns = get_columns(filters, salary_slips)
	...
	doj_map = get_employee_doj_map(filters)

	data = []
	for ss in salary_slips:
		if not filters.get('show_other_data'):
			row = [ss.name, ss.employee, ss.employee_name, doj_map.get(ss.employee).get('bank_name'),
				doj_map.get(ss.employee).get('bank_ac_no'), doj_map.get(ss.employee).get('ifsc_code'),
				ss.net_pay]
		else:
			row = [ss.name, ss.employee, ss.employee_name, doj_map.get(ss.employee).get('bank_name'),
				doj_map.get(ss.employee).get('bank_ac_no'), doj_map.get(ss.employee).get('ifsc_code'),
				ss.net_pay, doj_map.get(ss.employee).get('date_of_joining'),ss.branch, ss.department, ss.designation,
...
def get_columns(filters ,salary_slips):
	...
	if filters.get('show_other_data'):
		columns += [_("Date of Joining") + "::80", _("Branch") + ":Link/Branch:-1", _("Department") + ":Link/Department:-1",
...
def get_conditions(filters, company_currency):
	...
	if filters.get('show_model_bank'):
		conditions += " and bank_name = 'MODEL'"
	else:
		conditions += " and bank_name != 'MODEL'"
	return conditions, filters

def get_employee_doj_map(filters):
	if filters.get('show_model_bank'):
		cond = " where bank_name = 'MODEL'"
	else:
		cond = " where bank_name != 'MODEL'"

	employee_query	= frappe.db.sql("""
				SELECT
					employee, date_of_joining, bank_name, bank_ac_no, ifsc_code
				FROM `tabEmployee`
				""",as_dict=1)
	emp_map = {}
	for emp in employee_query:
		emp_map[emp.employee] = emp
