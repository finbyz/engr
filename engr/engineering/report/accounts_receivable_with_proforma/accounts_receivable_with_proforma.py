...
class ReceivablePayableReport(object):
	...
	def build_data(self):
		...
		if self.filters.show_proforma_invoices and self.filters.party_type == "Customer" and not self.filters.get('group_by_party'):
			self.get_proforma_entries()
			self.data = sorted(self.data, key= lambda x: (x.get("posting_date"), x.get("party")))
	...
	def get_proforma_entries(self):
		conditions = ""

		if self.filters.get("company"):
			conditions += f""" and pi.company = '{self.filters.get("company")}'"""

		if self.filters.get("customer"):
			conditions += f""" and pi.customer = '{self.filters.get("customer")}'"""

		if self.filters.get("sales_partner"):
			conditions += f""" and pi.customer in (select name from `tabCustomer` where default_sales_partner = '{self.filters.get('sales_partner')}')"""

		if self.filters.get("customer_group"):
			lft, rgt = frappe.db.get_value("Customer Group", self.filters.get('customer_group'), ["lft", "rgt"])
			conditions += """ and pi.customer in (select name from `tabCustomer`
		...
		proforma_query = frappe.db.sql("""
			select
				'Proforma Invoice' as voucher_type, pi.name as voucher_no, pi.transaction_date as posting_date, pi.currency,
				pi.payment_due_amount as invoiced, pi.advance_paid as paid, pi.customer as party, pi.company,
				(pi.payment_due_amount - pi.advance_paid) as outstanding,
				cu.customer_name, cu.customer_group, cu.territory, cu.customer_primary_contact
			from
				`tabProforma Invoice` as pi
				JOIN `tabCustomer` as cu on cu.name = pi.customer
			where
				pi.docstatus = 1 and pi.status not in ('Closed', 'Paid') and (pi.payment_due_amount - pi.advance_paid) > 1 {}
			order by pi.transaction_date, pi.customer
		""".format(conditions), as_dict= True)	

		if proforma_query:
			self.data.extend(proforma_query)
