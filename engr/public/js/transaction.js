...
erpnext.TransactionController = erpnext.taxes_and_totals.extend({
		...
		if (this.frm.fields_dict["items"].grid.get_field('blanket_order')) {
			this.frm.set_query("blanket_order", "items", function(doc, cdt, cdn) {
				...
				return {
					...
					filters: {
						"company": doc.company,
						"blanket_order_type": (in_list(['Sales Order'], doc.doctype)) ? "Selling" : "Purchasing",
						"item": item.item_code
					}
				}
			});
	...
	_get_item_list: function(item) {
		...
		var append_item = function(d) {
			if (d.item_code) {
				...
				if (in_list(["Quotation Item", "Sales Order Item", "Delivery Note Item", "Sales Invoice Item","Proforma Invoice Item"]), d.doctype){
	...
	set_gross_profit: function(item) {
		if (in_list(['Sales Order', 'Proforma Invoice'], this.frm.doc.doctype) && item.valuation_rate) {
	...
	blanket_order: function(doc, cdt, cdn) {
		var me = this;
		var item = locals[cdt][cdn];
		if (item.blanket_order && (in_list(['Sales Order'], item.parenttype)) || item.parenttype=="Purchase Order") {
			frappe.call({
